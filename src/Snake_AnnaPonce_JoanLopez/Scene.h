...
public:
	...
	inline SceneState GetState(void) const { return currentState; };								// que diferencia hay entre esto y 
	template <SceneState state> inline bool CheckState(void) const { return currentState == state; }; //
protected:
	SceneState currentState{ SceneState::NONE }; // Current state of the scene, NONE by default			ESTO
};